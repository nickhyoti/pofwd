...
import (
	...
	"os"
	"strings"
	f "github.com/nickhyoti/pofwd/funcs"
)
...
func main() {
	...
	if len(os.Args) == 2 {
		...
		if err := f.StartForwarding(os.Args[1], os.Args[2], os.Args[3], os.Args[4]); err != nil {
			log.Fatalln(err)
		}
		<-make(chan bool)
		os.Exit(0)
	...
	for confScanner.Scan() {
		...
		} else if err = f.StartForwarding(fields[0], fields[1], fields[2], fields[3]); err != nil {
			log.Fatalln(err)
		}
	}
	confFile.Close()
